...
public class ShiftScheduler {
     ...
     * @param multiEmpOneShift Whether or not to put more than one employee in a shift to compensate for work hours. Works only if `useWorkHours` is true.
     * @param multiShiftsOneEmp Whether or not employees can be assigned to more than one shift per day.
     * @param maxEmpsInShift Maximum number of employees in a single shift. Works only if `multiEmpOneShift` is true.
    ...
    public static Employee[][][] generateSchedule(
        ...
        if (maxEmpsInShift > 1 && multiEmpOneShift == false) throw new IllegalArgumentException("`maxEmpsInShift` works only if `multiEmpOneShift` is true");
        ...
        for (int day = 0; day < numDays; day++) {
            ...
            for (int shiftIndex = 0; shiftIndex < shifts.size(); shiftIndex++) {
                ...
                for (Employee employee : availableEmployees) {
                    ...
                    if (assignedEmployees.size() == maxEmpsInShift) break;
                }
        ...
        for (Employee employee : employees) {
            ...
            int totalWorkHours = totalWorkMinutes.get(employee) / 60;

            if (
                (assignedShifts < maxShifts || totalWorkHours < employee.getMinWorkHours())
                && totalWorkHours < employee.getMaxWorkHours()
            ) {
                // Add an extra shift for the under-assigned employee in a random slot
                if (VERBOSE) System.out.println("Rebalancing required for " + employee.getName() + " (" + assignedShifts + " shifts, " + totalWorkHours + " hours)");
                ...
                while (
                    ...
                    if (currentShift.contains(employee) || isOnHoliday(employee, holidays, currentDate)) continue;

                    boolean alreadyAssigned = multiShiftsOneEmp ? false : isAlreadyAssigned(employee, schedule, shifts, randomDay);
                    if (!alreadyAssigned && currentShift.size() < maxEmpsInShift) {
                        if (VERBOSE) System.out.println("EMPLOYEE ADDED: " + employee.getName());
                        ...
                        totalWorkMinutes.put(employee, totalWorkMinutes.get(employee) + shifts.get(randomShift).getLength());
                        totalWorkHours = totalWorkMinutes.get(employee) / 60;
                    }
                }
            }
        }
    ...
    private static void printSchedule(Employee[][][] employeeSchedule, List<Employee> employees, List<Shift> shifts) {
        ...
        for (Employee employee : employees) {
            int shiftCount = shiftCounts.getOrDefault(employee.getId(), 0);
            int workHour = workHours.getOrDefault(employee.getId(), 0);
            System.out.println(employee.getName() + ":\t" + shiftCount + " shifts\t" + workHour + " hours");
        }
    }
    ...
    public static void main(String[] args) {
        ...
        employees.add(new Employee(1, "Alice", 120, 173));
        employees.add(new Employee(2, "Bob", 120, 173));
        employees.add(new Employee(3, "Jack", 120, 173));
