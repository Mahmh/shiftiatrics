import pytest
from src.server.db import Session, create_account, create_schedule, delete_schedule, get_all_schedules_of_account, update_schedule, _check_schedule_requests
...
def test_invalid_schedule_requests(setup_and_teardown):
    ...
    with pytest.raises(ValueError):
        for i in range(SUB_INFO.plan_details.max_num_schedule_requests):
            schedule_id = create_schedule(account_id, **SCHEDULE).schedule_id
